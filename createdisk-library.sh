...
function copy_systemd_units() {
    ...
    ${SSH} core@${VM_IP} -- 'ls /home/core/systemd-units/*.path | xargs basename -a | xargs sudo systemctl enable'

    # enable only the units that donot have a .path counterpart
    ${SSH} core@${VM_IP} -- 'pushd /home/core/systemd-units; ls | grep -v -E $(find . -regex ".+\.path$" | xargs basename -a | sed "s/\.path//" | tr "\n" "|" | sed "s/.$//")'
