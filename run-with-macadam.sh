...
EOF
    ...
    if ! cloud-init schema -c "${USER_DATA_PATH}" --annotate; then
        die "Invalid cloud-config generated."
    fi
...
function ensure_gvproxy_exists() {
    echo "--- Setting up gvproxy ---"
    if command -v gvproxy &>/dev/null; then
        echo "gvproxy is already available in PATH."
        return
    fi

    if [ -f "/usr/bin/gvproxy" ]; then
        gvproxy_path="/usr/bin/gvproxy"
    elif [ -f "/usr/libexec/podman/gvproxy" ]; then
        gvproxy_path="/usr/libexec/podman/gvproxy"
    fi

    echo "gvproxy not found, downloading..."

    local version="latest"
    local url="https://github.com/containers/gvisor-tap-vsock/releases/download/${version}/gvproxy-linux-amd64"

    echo "Downloading from ${url}"
    if ! curl -fL -o /tmp/gvproxy "${url}"; then
        die "Failed to download gvproxy."
    fi
    sudo mv /tmp/gvproxy /usr/bin/gvproxy
    sudo chmod +x /usr/bin/gvproxy

    echo "gvproxy downloaded to /usr/bin"
}
...
# Ensures gvproxy binary has the required capability
function ensure_gvproxy_capability() {
    echo "--- Checking gvproxy capability ---"
    local gvproxy_path=""

    # Check for gvproxy in known locations
    if [ -f "/usr/bin/gvproxy" ]; then
        gvproxy_path="/usr/bin/gvproxy"
    elif [ -f "/usr/libexec/podman/gvproxy" ]; then
        gvproxy_path="/usr/libexec/podman/gvproxy"
    else
        die "gvproxy binary not found in /usr/bin or /usr/libexec/podman"
    fi

    echo "Found gvproxy at: ${gvproxy_path}"

    # Check if the capability is already set
    local current_caps
    current_caps=$(getcap "${gvproxy_path}" 2>/dev/null || true)

    if echo "${current_caps}" | grep -q "cap_net_bind_service=.*ep"; then
        echo "gvproxy already has cap_net_bind_service=+ep capability set."
        return
    fi

    echo "Setting cap_net_bind_service=+ep capability on gvproxy..."
    if ! sudo setcap cap_net_bind_service=+ep "${gvproxy_path}"; then
        die "Failed to set capability on gvproxy. Please ensure you have sudo privileges."
    fi

    # Verify the capability was set
    current_caps=$(getcap "${gvproxy_path}" 2>/dev/null || true)
    if echo "${current_caps}" | grep -q "cap_net_bind_service=.*ep"; then
        echo "Capability successfully set on gvproxy."
    else
        die "Failed to verify capability was set on gvproxy."
    fi
}

# Exposes ports from the VM to the host
function forward_port_gvproxy() {
    echo "--- Exposing ports ---"
    local ports=("6443" "443" "80")
    for port in "${ports[@]}"; do
        local expose_req="{\"local\":\"127.0.0.1:${port}\",\"remote\":\"192.168.127.2:${port}\",\"protocol\":\"tcp\"}"
        ...
        if ! curl -f \
            ...
            die "Failed to forward port: ${expose_req}"
        fi
    done
}
